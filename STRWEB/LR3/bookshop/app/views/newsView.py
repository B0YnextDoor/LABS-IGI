    ...
    def post(self, request):
        ...
        if (request.POST.get('action') == 'add'):
            return render(request, self.template_name, {'role': request.session.get('role'), 'news': None,
                                                        'form': NewsForm(), 'cart': request.session.get('cart')})
...
class NewsDetailView(View):
    ...
    def get(self, request, id):
        ...
        return render(request, self.template_name, {'role': request.session['role'], 'news': news, 'form': None, 'cart': request.session.get('cart')})
    ...
    def post(self, request, id):
        ...
        if (request.POST.get('action') == 'upd'):
            ...
            return render(request, self.template_name, {'role': request.session['role'], 'news': news, 'form': form, 'cart': request.session.get('cart')})

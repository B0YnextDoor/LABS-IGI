from app.repositories.commonRepository import InfoRepository, NewsRepository, \
    QARepository, VacancyRepository, PartnerRepository, BannerRepository
...
class BannerService:
    @staticmethod
    def get():
        data = BannerRepository.get()
        return {
            'loop': data.loop,
            'navs': data.navs,
            'pags': data.pags,
            'auto': data.auto,
            'stopMouseHover': data.stopMouseHover,
            'delay': data.delay
        }

    @staticmethod
    def update(loop: bool, navs: bool, pags: bool, auto: bool, stopMouseHover: bool, delay: float):
        result = BannerRepository.update(
            loop, navs, pags, auto, stopMouseHover, delay)
        return BannerService.get() if result is not None else None

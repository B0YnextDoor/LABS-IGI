...
class AdminService:
    ...
    @staticmethod
    def get_price_list():
        AdminService.books = bookService.BookService.get_all_admin()
        return AdminService.books

    @staticmethod
    def draw_customers_stat(customers):
        x = customers.keys()
        y = [len(value) for value in customers.values()]
        ...
        plt.clf()
        AdminService.graph_data['cityList'] = list(x)
        AdminService.graph_data['cityCount'] = y
    ...
    def draw_month_stats(months, sales, incomes):
        AdminService.graph_data['month'] = months
        AdminService.graph_data['sales'] = sales
        AdminService.graph_data['incomes'] = incomes
    ...
    def get_forecast():
        ...
        plt.plot(months, AdminService.sales, marker='o', markersize=5,
                 linestyle='-', label='Sales trend')
        plt.xlabel('Months')
        plt.ylabel('Sales amount')
        ...
        plt.clf()
        return ceil(sum(AdminService.sales) / len(AdminService.sales))

...
class OrderRepositoryTest(TestCase):
    ...
    def test_get_user_orders(self):
        ...
        orders = OrderRepository.get_user_orders(user)
        self.assertIsNotNone(orders)
        self.assertEquals(len(orders), 1)
        self.assertEquals(orders[0]['id'], 1)
        self.assertEquals(orders[0]['status'], '1')
    ...
    def test_get_by_id(self):
        order = OrderRepository.get_by_id(1)
        self.assertIsNotNone(order)
        self.assertEquals(order['status'], '0')

    def test_create_order(self):
        new_odrer = OrderRepository.create_order(
    ...
    def test_cancel_order(self):
        order = OrderRepository.cancel_order(1)

...
class UserServiceTest(TestCase):
    ...
    def test_get_by_email(self):
        user = UserService.get_by_email('test@customer.com')
    ...
    def test_get_review_by_id(self):
        rew = UserService.get_review_by_id(1)
    ...
    def test_create_review(self):
        rew = UserService.create_review('test@customer.com', 4, 'Aboba')
        self.assertIsNotNone(rew)
        self.assertEquals(rew.user.id, 1)
    ...
    def test_update_review(self):
        rew = UserService.update_review(1, 5, 'Upd text')
        self.assertIsNotNone(rew)
        self.assertEquals(rew.user.id, 1)
        self.assertEquals(rew.rate, 5)
    ...
    def test_delete_review(self):
        rew = UserService.delete_review(1)

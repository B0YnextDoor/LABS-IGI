...
class QAView(View):
    ...
    def get(self, request):
        return render(request, self.template_name, {'role': request.session['role'], 'qas': QAService.get_all(), 'form': None, 'action': None})

    def post(self, request):
        if request.session.get('role') != 'adm':
            return redirect('faq')
        if (request.POST.get('action') == 'add'):
            return render(request, self.template_name, {'role': request.session['role'], 'qas': None, 'form': QAForm(), 'action': 'add'})
            ...
            return render(request, self.template_name, {'role': request.session['role'], 'qas': None, 'form': form, 'action': id})
        ...
        if form.is_valid():
            try:
                ...
                form.add_error(None, f'DB error')
        return render(request, self.template_name, {'role': request.session['role'], 'qas': None, 'form': form, 'action': action})
